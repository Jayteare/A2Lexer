		...
		******************************************************************/
		void result_Stack_Out()
		{
			...
			for (int i = 0; i < result_Stack.size(); i++)
			{
				...
				//
				if (Pgm) { grammer_Stack.push_back("(N: rule= #1: Pgm = kwdprog brace1 Stmts brace2)"); }
				if (Slist) { grammer_Stack.push_back("(N: rule= #2: Stmts = Stmt semi Stmts)"); }
				if (Stmt) { grammer_Stack.push_back("(N: rule= #3: Stmt = kwdprint parens1 Elist parens2)"); }
				if (S_Asgn) { grammer_Stack.push_back("(N: rule= #5: S_Asgn = id equal E)"); }
				if (S_In) { grammer_Stack.push_back("(N: rule = #6: S_In = id equal kwdinput)"); }
				if (S_Out) { grammer_Stack.push_back("(N: rule = #7:S_Out = kwdprint paren1 Elist paren2)"); }
				if (Elist) { grammer_Stack.push_back("(N: rule= #8: Elist = E comma Elist)"); }
				if (Elist2) { grammer_Stack.push_back("(N: rule= #9: Elist2 = E Elist2 | eps)"); }
				if (E_) { grammer_Stack.push_back("(N: rule= #10: E = E Opadd T | T)"); }
				if (T_){ grammer_Stack.push_back("(N: rule= #11: T = T Opmul F | F)"); }
				if (F_) { grammer_Stack.push_back("(N: rule= #12: F = Fatom | paren1 E paren2)"); }
				if (Fatom) { grammer_Stack.push_back("(N: rule= #13: Fatom = id | int | float | string"); }
				if (Opadd) { grammer_Stack.push_back("(N: rule = #14: Opadd = plus | minus)"); }
				if (Opmul) { grammer_Stack.push_back("(N: rule = #15: Opmul = aster | slash | caret)"); }
			...
			myfile << " Grammatical Analysis of Test Case Using a PST" << endl;
			string _out_g   = "";
			...
			myfile << " Grammatical Analysis of Test Case Using a AST" << endl;
			string _out_AST = "";
			for (int index = (grammer_Stack.size()-1); index > 0; index--)
			{
				_out_AST += grammer_Stack[index] + "\n ";
			}

			myfile << _out_AST << endl;
			cout   << _out_AST << endl;
